...
// command right before the target command and take everything after its
// (deterministic, single-line) output.
const SENTINEL = 424243;
const SENTINEL_COMMAND = `parse in NAT : ${SENTINEL} .`;
const SENTINEL_OUTPUT = `NzNat: ${SENTINEL}`;
 ...
 * parses the command's textual answer into structured data.
 *
 * ```ts
 * const m = new Maude();
